...
class SendData
{
    ...
    public static function closeAtherClient($user_id,$client_id){
        if($client_ids = Gateway::getClientIdByUid($user_id)){
            foreach ($client_ids as $value) {
                if($value == $client_id)continue;
                self::offline($value);
            }
        }
    }
     ...
     * @param array  $data    发送的数据
     */
    public static function sendToClient($client_id, $name, $data)
    {
        Gateway::sendToClient($client_id, self::formatData($name, $data));
    }
